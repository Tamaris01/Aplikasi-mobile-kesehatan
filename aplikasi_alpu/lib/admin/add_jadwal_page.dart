...
class _AddJadwalPageState extends State<AddJadwalPage> {
  ...
  void _showConfirmationDialog(BuildContext context, VoidCallback onConfirm) {
    AwesomeDialog(
      ...
      title: 'Konfirmasi',
      desc: 'Apakah Anda yakin ingin menyimpan jadwal dokter ini?',
      btnCancelOnPress: () {
        // Jika tombol "Tidak" ditekan
        print('Tidak disimpan');
      },
      btnCancelText: 'Tidak',
      btnCancelColor: Colors.red, // Warna tombol "Tidak"
      btnOkOnPress: () {
        // Jika tombol "Ya" ditekan
        onConfirm();
      },
      btnOkText: 'Ya',
      btnOkColor: const Color.fromARGB(255, 8, 88, 153), // Warna tombol "Ya"
    ).show();
  }

  void _onSavePressed() {
    _showConfirmationDialog(context, () async {
      await _addSimpanJadwalDokter();
    });
  }
...
// Metode untuk menampilkan dialog sukses
  void _showSuccessDialog(
    ...
    AwesomeDialog(
      ...
      desc: message,
      btnOkOnPress: () {
        onDismiss(); // Memanggil callback setelah tombol OK ditekan
      },
    ).show();
  }
...
// Metode untuk menampilkan dialog error
  ...
  Future<void> _addSimpanJadwalDokter() async {
    ...
    if (response.statusCode == 200) {
      ...
      _showSuccessDialog(context, 'Data Jadwal Berhasil tersimpan', () {
        Navigator.pop(
            context); // Kembali ke halaman sebelumnya setelah dialog ditutup
      });
    } else {
      print('Data Jadwal gagal tersimpan');
      _showErrorDialog(context, 'Data Jadwal gagal tersimpan');
    }
  }
